$(function () {
    ...
    $(".menu li a[href^='#'], .m_menu li a[href^='#']").on("click", function () {
        ...
        // html,body 동시에 잡아주는 이유는 웹 호환성때문
    });

    // 햄버거버튼 클릭시 - 모바일 메뉴 보기/가리기
    // 햄버거버튼 클릭시 메뉴박스와 x버튼 보기 
 ...
 $(window).scroll(function () { // .scroll 스크롤 이벤트   
        ...
        // 포폴배경 위치이동
        $(".port_bg li").eq(0).css({
            //  y(수직) 위치값을 초기 위치보다 위로 알라가도록 효과줌
...
// 스크롤이 위로 올라갈수록 li의 배경 이미지들이 동적으로 이동되는 스크립트
...
//  스크롤 위치에 따라 동적으로 계산된 값에 따라 위치이동이 이루어짐
